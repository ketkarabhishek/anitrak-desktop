...
export async function GetAllEntries(userId, token){
    const url = baseUrl + "/" + userId + "/library-entries?filter[kind]=anime&include=anime.genres&page[limit]=500"
...
function makeRequest(url, token){
    return new Promise((resolve, reject) => {
        ...
        fetch(url, options).then((res) => res.json()).then((resJson) => {
...
async function extractEntryFromJson(resJson, keys){
    ...
    for (let index = 0; index < totalItemCount; index++) {
        ...
        entry.reconsumeCount = parseInt(resJson.data[key]['attributes']['reconsumeCount']);
        entry.ratingTwenty = parseInt(resJson.data[key]['attributes']['ratingTwenty']);
        entry.averageRating = parseInt(resJson.data[key]['attributes']['averageRating'])
        ...
        entry.posterUrl = resJson.included[key]['attributes']['posterImage']['small'];
        entry.total = parseInt(resJson.included[key]['attributes']['episodeCount']);
        entry.episodeLength = parseInt(resJson.included[key]['attributes']['episodeLength']);
        ...
        var categories = resJson.included[key]['relationships']['genres']['data'];
...
export function UpdateKitsuEntry(kitsuAuth, data){
    return new Promise(async (resolve, reject) => {
        const body = {
            data: 
            {
                type: "libraryEntries", id: data.id,
                attributes: 
                {
                    status: data.status,
