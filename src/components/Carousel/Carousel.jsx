...
export default function Carousel(){
  ...
  const DataDisplay = ({empty}) => empty > 0 ?
      (
      ...
      :(
        <div>
           {isLoading 
            ? (
              <Grid container justify="center" alignItems="center" style={{height: 390}}><CircularProgress size="4rem" disableShrink/></Grid>
               
            )
            
            :(
                <MyCarousel responsive={responsive} centerMode={true} slidesToSlide={2}
                    draggable={true} swipeable={false} itemClass="carousel-cell" containerClass="carousel-container"
                    >
                        {carouselItems}
                </MyCarousel>
            )
