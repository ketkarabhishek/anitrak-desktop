            ...
            try{
                ...
                Website.findOne({siteName: siteName}, (err, site) => {
                    if(err){
                        ...
                        Website.insert({siteName: siteName, userId: userId, userName: userName, password: password, sync: true}, (err, newSite) => {
                            if(err){
                                console.error(err);
                            }
                            else{
    ...
    return new Promise((resolve, reject) => {
        Website.findOne({siteName: 'kitsu'}, (err, doc) => {
            if(err){
                ...
                resolve({siteName: 'kitsu', userId: "", userName: "", password: "", sync: false})
            }
        })
    })
}
...
export function GetLibrary(query){
    return new Promise((resolve, reject) => {
        Library.find(query).sort({updatedAt: -1}).exec((err, entries) => {
            if(err){
                console.log(err)
            }
            else{
...
export function AddKitsuDetails(id, response){
    Library.findOne({kitsuId: id}, (err, doc) => {
        if(err){
            ...
            doc.save((err, newDoc) => {console.log(JSON.stringify(newDoc))});
        }
    });
}
        ...
        Library.findById(updatedEntry._id, (entry) => {
            ...
            const queueItem = {
                ...
                data: {
                    ...
                    ratingTwenty: updatedEntry.ratingTwenty,
                },
                isSynced: false,
            }

            console.log("Updated db entry: " + updatedEntry.title + "-" + updatedEntry.progress)
...
export function GetTopGenres(){
    return new Promise(async (resolve, reject) => {
        ...
        if(total > 0){
            Library.find({status: {$in :['completed', 'current']}}, async (err, docs) => {
                var ar = [];
                docs.forEach((item) => {
...
export function GetTotalWatchTime(){
    return new Promise((resolve, reject) => {
        Library.find({status: {$in :['completed']}}, (err, docs) => {
            ...
            let days = Math.floor(((totalMin % (60 * 24 * 365)) % 43800.048) / (60 *24));
...
export function ToggleQueueItemStatus(id){
    Queue.findById(id, (err, doc) => {
        if(err){
            ...
            doc.isSynced = true;
            doc.save((err) => {if(err)console.error(err)} );;
        }
    })
}